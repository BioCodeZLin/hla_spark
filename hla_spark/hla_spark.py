...
def arg_init1(sample_name, fastq1, fastq2, output_dir, n,step_list,hlatype):
    global sample
    global fq1
    global fq2
    global outdir
...
def HLA_reads_extraction_byGene_module(gene):
    
    cmd = []
    cmd.append(
        f'\n------------------------------HLA reads extraction for HLA gene: {gene}----------------------------')
...
def Bestm_judge(gene):
    if hlatypes=='auto':
        Best_matched_alleles_module(gene)
    else:
        df_hlatypes=pd.read_csv(hlatypes,sep='\t',index_col=0)
        sams=df_hlatypes.index.values
        cols=list(df_hlatypes)
        allele1=gene+'1'
        allele2=gene+'2'
        if sample not in sams:
            Best_matched_alleles_module(gene)
            return
        if (allele1 in cols) and (allele2 in cols):
            type1=df_hlatypes.loc[sample,allele1]
            type2=df_hlatypes.loc[sample,allele2]
            if (type1=='-') or (type2=='-'):
                Best_matched_alleles_module(gene)
            else:
                ...
                if type1==type2:
                    ht=df_ref_list[df_ref_list['name_brief']==type1].index.values
                    if len(ht)>=1:
                        hla_types=[ht[0]]
                    else:
                        Best_matched_alleles_module(gene)
                    ...
                    ht1=df_ref_list[df_ref_list['name_brief']==type1].index.values
                    ht2=df_ref_list[df_ref_list['name_brief']==type2].index.values
                    if len(ht1)>=1 and len(ht2)>=1:
                        hla_types=[ht1[0],ht2[0]]
                    else:
                        Best_matched_alleles_module(gene)
                        return
                ...
                dir_gene = mkdir(f"{dir_snp_calling}/{gene}/")
                dir = mkdir(f"{dir_gene}/00.fastq")
                bestmatch = f'{dir}/bestmatch'
                gene_fasta = f"{dir}/{gene}_bestm.fasta"
                best_m.ref_construction(hla_types, df_ref_list, gene_fasta, bestmatch, 2, sample,config.get('soft', 'BWA'))
        else:
            Best_matched_alleles_module(gene)
...
def gene_jobs(gene):
    ...
    start_time = startTime()
    if "1" in steps_list:
        HLA_reads_extraction_byGene_module(gene)
    if "2" in steps_list:
        Bestm_judge(gene)
...
def run_hla_spark(sample_name, fastq1, fastq2, output_dir="hla_spark_results/", n_core=4,step_list="1,2,3,4,5,6,7,8",hla_genes='all',hlatype='auto'):
    arg_init1(sample_name,fastq1,fastq2,output_dir,n_core,step_list,hlatype)
    arg_init2()
    try:
        if hla_genes=='all':
        ...
        if ("1" in steps_list) or ("2" in steps_list) or ("3" in steps_list) or ("4" in steps_list) or ("5" in steps_list) or (
                "6" in steps_list):
